<?php 
	...
	{
		$alert='';
		if (empty($_POST['producto']) || empty($_POST['nombre']) || empty($_POST['precio']) || empty($_POST['id']) || empty($_POST['foto_actual']) || empty($_POST['foto_remove']))
		{
			...
			if ($nombre_foto != '') {
				...
				$query_update = mysqli_query($conection,"UPDATE productos
	...
	if (empty($_REQUEST['id'])) {
		...
		$query_producto = mysqli_query($conection,"SELECT p.Id_producto,p.producto,pr.nombre,p.precio,p.foto 
													FROM productos p
...
<body>
	...
	<section id="container">
		...
		<div class="form_register">
			<h1><i class="fas fa-plus"></i> Editar Producto</h1>
			<hr>
			...
			<form action="" method="post" enctype="multipart/form-data">
				...
				<input type="text" name="producto" id="producto" placeholder="Nombre" value="<?php echo $data_producto['producto']; ?>">
				...
				<select name="nombre" id="nombre" class="notItemOne">
					<option value="<?php echo $data_producto['id_proveedor']; ?>" selected><?php echo $data_producto['nombre']; ?></option>
					<?php 
				...
				<button type="submit" class="btn_save"><i class="fas fa-save"></i> Editar Producto</button>
			</form>
